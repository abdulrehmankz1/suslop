...
// Component changed to use NewsRoom functions
const NewsRoomDetailPage = async (props: {
  params: Promise<{ slug: string }>;
}) => {
  const { slug } = await props.params;

  // Fetch data using the NewsRoom service
  const newsRoom = await fetchNewsRoomBySlug(slug);
  if (!newsRoom) {
    notFound();
  }

  const newsRoomData = extractNewsRoomData(newsRoom);
  if (!newsRoomData) {
    notFound();
  }
...
export async function generateStaticParams() {
  const newsRooms = await fetchAllNewsRooms();
  return newsRooms.map((newsRoom) => ({
    slug: newsRoom.slug,
  }));
}
